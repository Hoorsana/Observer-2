...
class TestTimeSeries:
    ...
    @pytest.mark.parametrize('ts, time, expected', [
        pytest.param(
        ...
        pytest.param(
            timeseries.TimeSeries([0, 1, 2], [[0, 1], [2, 3], [4, 5]], kind='linear'),
            -1,
            timeseries.TimeSeries([-1, 0, 1, 2], [[-2, -1], [0, 1],
                                  [2, 3], [4, 5]], kind='linear')
        ),
        pytest.param(
            timeseries.TimeSeries(
    ...
    @pytest.mark.parametrize('ts, expected', [
        pytest.param(
            timeseries.TimeSeries([0, 1, 2], [[2.0], [3.5], [5.1]]),
            (1,)
        ),
        pytest.param(
            timeseries.TimeSeries(
                [-3, -2, -1],
                [[2.0, 3.0, 1.5], [3.5, 2.0, 1.0], [5.1, 4.3, 2.3]]
            ),
            (3,)
        ),
        pytest.param(
