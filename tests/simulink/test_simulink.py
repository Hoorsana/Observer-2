...
class TestTest:
    @pytest.mark.parametrize(
        "code, requests, results, severity, effect",
        [
            pytest.param(
                "error('foo');",
                [],
            ...
            pytest.param(
                ...
                [
                    simulink._LoggingRequest(
                        infos.LoggingInfo(target="foo", signal="bar"), lambda x: x
                    )
                ],
                {},
                "",
...
class TestLoadDetails:
    @pytest.mark.parametrize("side_effect", [OSError, FileNotFoundError])
...
def limit():
    quick_pulse = infos.PhaseInfo(
        ...
        description=(
            "Send a short burst of high temperature to test the "
            ...
            infos.TargetInfo(
                ...
                signals=[
                    ...
                    infos.SignalInfo(
                        ...
                        range=infos.RangeInfo(min=0, max=100),
                        description=(
                            "Return 1 if the temperature limit"
