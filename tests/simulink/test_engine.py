...
def simulation(engine):
    return engine._engine.sim("test_engine")


@pytest.mark.slow
@pytest.mark.dependency("simulink")
@pytest.mark.parametrize(
    "var, expected0, expected1",
    [
...
def test_timeseries_to_python_from_simulink_model(
    var, expected0, expected1, simulation, engine
):
    obj = engine.get_field(simulation, var)
    ts = engine.timeseries_to_python(obj)
